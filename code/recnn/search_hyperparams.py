...
#File with the input trees

#sample_name = str(sys.argv[1])
#jet_algorithm= str(sys.argv[2])
...
parser.add_argument('--sample_name',  help="Sample name", default=sample_name)
...
#data_dir='../data/inputTrees/'+args.sample_name
#-------------------------------------------------------------------------------------------------------------
#//////////////////////    FUNCTIONS     //////////////////////////////////////////////
#-------------------------------------------------------------------------------------------------------------
#------------------------------------------
...
#      print('old_parent_dir= '+args.parent_dir)
      parent_dir=args.parent_dir+str(dir_name)+'/'
#      print('new_parent_dir= '+parent_dir)
...
#      print(jet_algorithm, jet_numbers, hidden_dims,num_epochs,batch_sizes,decays,learning_rates)
                ...
                for batch_size in batch_sizes:
                  for decay in decays: 
                    for learning_rate in learning_rates:
                      print(jet_algo, jet_number, hidden_dim,num_epoch,batch_size,decay, learning_rate)
                      params.jet_algo=jet_algo
                      ...
                      params.name=name #This goes into the name of the batched dataset that we use to train/evaluate/test
                      params.nrun_start=Nrun_start
                      params.nrun_finish=Nrun_finish
                      print('PARAM_check_2'+str(params))
                  #-----------------------------------------
                  # Launch job (name has to be unique)
                      ...
                      print('output dirctory name (ish) = '+str(job_name))
                 
                  
                  #-----------------------------------------                
                  # Run preprocess, training, evaluation 
                      if PREPROCESS:
                        launch_preprocessing_job(parent_dir, args.data_dir, job_name, params, jet_algo)
                  
                      if TRAIN_and_EVALUATE:
                        for n_run in np.arange(Nrun_start,Nrun_finish):
                          launch_training_job(parent_dir, args.data_dir, args.eval_data_dir, job_name+'/run_'+str(n_run), params, args.gpu, sample_name, jet_algo)        

                          launch_evaluation_job(parent_dir, args.data_dir, args.eval_data_dir, job_name+'/run_'+str(n_run), params, args.gpu, sample_name, jet_algor)
...
multi_scan(jet_algorithm=[args.jet_algorithm],
        learning_rates=[.002],
        decays=[0.9], 
        batch_sizes=[64],
        num_epochs=[40],
        hidden_dims=[64,128,156,512,1024], 
        jet_numbers=[120000],
        Nfeatures=7,
        dir_name=args.jet_algorithm,
        name=args.jet_algorithm, 
        info='', 
